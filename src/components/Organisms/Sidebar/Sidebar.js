...
const Sidebar = () => {
  ...
  useEffect(() => {
    ...
    for (const item of menu) {
      if (
        item.path === pathname ||
        pathname.includes(item?.path?.replace(':id', ''))
      ) {
        temp = {
      ...
      if (item.children?.length) {
        for (const itemSub of item.children) {
          if (
            itemSub.path === pathname ||
            pathname.includes(itemSub?.path.replace(':id', ''))
          ) {
            temp = {
    ...
    return (
      <Menu
        ...
        onOpenChange={(openKeys) =>
          setMenuSelected({ ...menuSelected, openKey: openKeys })
        }
        expandIcon={({ isOpen }) => {
          return (
    ...
    return (
      <Fragment key={data.path}>
        <Menu.Item
          ...
          icon={
            <Icon
              component={isSub ? RightOutlined : data.icon}
